  ...
  listen: function(hash){
    for(var type in this.events){
      ...
      if(el.socket){
        ...
        el.specific_socket.disconnect();
      }
      el.socket = el.specific_socket = null;
      el.data = null;
    ...
    for(var type in hash){
      ...
      el.connectData(obj.key, obj.specific_key, obj.data)
    }

    $DATA = {}
    for(var i in this.events){
      $DATA[i]=this.events[i].data;
    }
  },
...
NotificationElement.prototype = {
  ...
  connectData: function(key, specific_key, data){
    this.disconnect();
    this.data = data;
    this.socket = new IOSocket('/notification', key);
    this.socket.ondata = this.ondata.bind(this);
    this.specific_socket = new IOSocket('/notification', specific_key);
    this.specific_socket.ondata = this.ondata.bind(this);
  },
  disconnect: function(){
    this.data=null;
    if(this.socket){
      this.socket.disconnect();
      this.specific_socket.disconnect();
    }
    this.socket = this.specific_socket = null;
  },
  ...
  applyChildChange: function(keys, data, prev, changes){
    ...
    if(typeof data == 'object'){
      var p = pdata && pdata[key];
      var pclone = p;
      if(typeof p == 'object'){
        pclone={};
        for(var i in p)pclone[i]=p[i];
      }
      odata[key] = p||{};
      for(var i in data)odata[key][i]=data[i];
      if(pdata)pdata[key] = pclone;
    }else odata[key] = data;
